...
def borrar_imagenes_nocturnas (path_directory, amanecer, atardecer):
    ...
    for i in range(len(Folders)):
        for file in os.listdir(path_directory+Folders[i]+'/recortados'):
            if file.endswith(".nc"):
                fecha_utc, fecha_path, fecha, fecha_local = retorna_fecha_hora_info(path_directory+Folders[i]+"/recortados/" + file)
                ...
                if local_hour > dt.datetime.strptime(atardecer, '%H:%M').time() or local_hour < dt.datetime.strptime(
                        amanecer, '%H:%M').time():
                    os.system('rm ' + path_directory+Folders[i]+"/recortados/" + file)
