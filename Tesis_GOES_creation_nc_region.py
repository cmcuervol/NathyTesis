...
path_directory = "/home/nacorreasa/Maestria/Datos_Tesis/GOES/GOES_CH2_2019/"
path_save = '/home/nacorreasa/Maestria/Datos_Tesis/GOES/GOES_nc_CREADOS/'
Name_nc = 'GOES_VA_C2_2019'
atardecer = '18:20'
amanecer = '05:40'
Banda = '02'
resolucion = '0.5 km'
...
for single_date in daterange(fi, ff):
    Folders.append(single_date.date().strftime('%Y%m%d') + '_' + Banda)
...
for folder in Folders:
    if os.path.exists(path_directory+folder+'/') == True:
        #dias =  Listador(path_directory+folder+'/', 'OR_ABI-L1b-RadF-M3C'+str(Banda), '.nc')
        dias =  Listador(path_directory+folder+'/', 'OR_ABI-L1b-RadF-M', '.nc')
        dias.sort()
        for hora in dias:
...
Radiancias  = zeros((len(archivos), shape(rad)[1], shape(rad)[2]))
a.close()
...
for i  in range(len(archivos)):
    ...
    Radiancias_temp = a_i.variables['Radiancias']
...
nw.title =  'GOES para el Valle de Aburra-Banda:' +str(Banda)
